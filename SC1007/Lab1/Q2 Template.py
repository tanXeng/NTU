...
def removeNode(ll, index):
    cur = ll.head
    if index == 0: # handle the special case where index == 0
    ...
    while index > 0:
        ...
        cur = cur.next
        index -= 1
        if cur is None: # ensure that the index does not go out of range
            print("Index out of range\nThe node cannot be removed.")
            return ll
    pre.next = cur.next # forget about cur
    del cur # remove cur from memory
    ll.size -= 1
    return ll
